...
export class SupportSystem {
  ...
  constructor() {
    ...
    // Use a more browser-friendly way to determine environment
    this.API_BASE_URL = typeof window !== 'undefined' && window.location.hostname === 'localhost'
      ? 'http://localhost:3000/support'
      : 'https://api.bytescookies.com/support';
  }

  async createTicket(ticket: Omit<SupportTicket, 'id' | 'createdAt' | 'updatedAt' | 'status'>): Promise<SupportResponse> {
    try {
  ...
  async updateTicket(ticketId: string, updates: Partial<SupportTicket>): Promise<SupportResponse> {
    try {
  ...
  async addComment(ticketId: string, comment: Omit<SupportComment, 'id' | 'createdAt'>): Promise<SupportResponse> {
    try {
  ...
  async getTicket(ticketId: string): Promise<SupportResponse> {
    try {
      const response = await this.fetchWithRetry(`${this.API_BASE_URL}/tickets/${ticketId}`);
  ...
  async getUserTickets(userId: string): Promise<SupportResponse> {
    try {
